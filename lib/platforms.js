...
/* Endpoint bases are overridable via env so the whole OAuth flow can be
 * exercised against a mock provider in tests. Defaults are the real thing. */
...
const YT_API = process.env.GOOGLE_API_BASE || 'https://www.googleapis.com/youtube/v3';
const FB_AUTH_URL = process.env.FB_AUTH_URL || 'https://www.facebook.com/v21.0/dialog/oauth';
const FB_API = process.env.FB_API_BASE || 'https://graph.facebook.com/v21.0';
...
/** The admin key authorizes changing already-saved platform credentials from
 *  the dashboard wizard. It lives in .env and the startup log — having it
 *  proves server access. Generated on first boot if absent. */
function ensureAdminKey() {
  if (!process.env.ADMIN_KEY) {
    process.env.ADMIN_KEY = crypto.randomBytes(9).toString('base64url');
    persistEnv({ ADMIN_KEY: process.env.ADMIN_KEY });
  }
...
async function youtubeExchangeCode(code, redirectUri) {
  const tok = await httpJson(GOOGLE_TOKEN_URL, {
    method: 'POST',
  ...
  try {
    await httpJson(`${YT_API}/liveStreams?part=id&mine=true&maxResults=1`,
      { headers: { Authorization: `Bearer ${tok.access_token}` } });
  } catch (e) {
...
async function youtubeEnd(conn, broadcastId) {
  try {
    const token = await youtubeToken(conn);
    await httpJson(
      `${YT_API}/liveBroadcasts/transition?broadcastStatus=complete&id=${broadcastId}&part=status`,
...
module.exports = {
  configured, setCredentials, ensureAdminKey,
